import { ActivityIcon, HomeIcon, MoonIcon, PersonStanding, SunIcon, UserIcon, UtensilsIcon } from "lucide-react"
    ...
    border-slate-100 dark:border-slate-800 p-6 transition-colors duration-200">
        ...
        <div className="flex flex-col gap-2">
            {navItems.map((item)=>(
                ...
                className={({isActive})=>`flex items-center gap-3 px-4 py-2.5 border-1-3 
        ...
        <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
            <button 
            onClick={toggleTheme}
            className="flex items-center gap-3 px-2 py-2.5 w-full text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800
